    ...
    const syncJobs = () => {
      try {
        ...
        const jobs = allJobs.map((job) => ({
          ...
          submissionType: (job as Job).submissionType,
          vendorAccess: (job as Job).vendorAccess,
          allowedVendorIds: (job as Job).allowedVendorIds,
          vendorNotes: (job as Job).vendorNotes,
          publishVisibility: (job as Job).publishVisibility,
          publishStartDate: (job as Job).publishStartDate,
          publishEndDate: (job as Job).publishEndDate,
          notifyVendorsOnPublish: (job as Job).notifyVendorsOnPublish,
          notifyCandidatesOnPublish: (job as Job).notifyCandidatesOnPublish,
  ...
  const createJob = useCallback(
    async (payload: {
      ...
      const job: Job = {
        ...
        submissionType: (dbJob as Job).submissionType,
        vendorAccess: (dbJob as Job).vendorAccess,
        allowedVendorIds: (dbJob as Job).allowedVendorIds,
        vendorNotes: (dbJob as Job).vendorNotes,
        publishVisibility: (dbJob as Job).publishVisibility,
        publishStartDate: (dbJob as Job).publishStartDate,
        publishEndDate: (dbJob as Job).publishEndDate,
        notifyVendorsOnPublish: (dbJob as Job).notifyVendorsOnPublish,
        notifyCandidatesOnPublish: (dbJob as Job).notifyCandidatesOnPublish,
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          const updatedJobs = allJobs.map((j) => ({
            ...
            submissionType: (j as Job).submissionType,
            vendorAccess: (j as Job).vendorAccess,
            allowedVendorIds: (j as Job).allowedVendorIds,
            vendorNotes: (j as Job).vendorNotes,
            publishVisibility: (j as Job).publishVisibility,
            publishStartDate: (j as Job).publishStartDate,
            publishEndDate: (j as Job).publishEndDate,
            notifyVendorsOnPublish: (j as Job).notifyVendorsOnPublish,
            notifyCandidatesOnPublish: (j as Job).notifyCandidatesOnPublish,
