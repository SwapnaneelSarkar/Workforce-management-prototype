...
import { vendorNavLinks } from "@/lib/navigation-links"

export default function VendorLayout({ children }: { children: ReactNode }) {
  const pathname = usePathname()
  const isLoginScreen = pathname === "/vendor/login"
  ...
  return (
    <div className="flex min-h-screen bg-background">
      <Sidebar
        items={vendorNavLinks}
        header={
          <div>
            <p className="text-xs font-semibold uppercase text-muted-foreground">Vendor</p>
            <p className="text-base font-semibold text-foreground">Partner Console</p>
          </div>
        }
        footer={
          <Link
